...
public class GetSingleInfomationClass {

	public static List<DetailInfor> GetSingleInformation(String idUser, String partner, String idHotel) {
		try {
			String data = BasicService
					.getData(Config.URLADDRESS + "/rating/" + idHotel + "?user=" + idUser + "&partner=" + partner);
			System.out.println(Config.URLADDRESS + "/rating/" + idHotel + "?user=" + idUser + "&partner=" + partner);
			JSONObject jsonObject = new JSONObject(data);
			List<DetailInfor> listInfor = new ArrayList<>();
			try {
				JSONArray listHotelJson = jsonObject.getJSONArray("InformationRating");

				for (int i = 0; i < 6; i++) {
					JSONObject rec = listHotelJson.getJSONObject(i);
					DetailInfor inforRating = new DetailInfor();
					inforRating.setRating(rec.getLong("rating"));
					inforRating.setPrice(rec.getLong("price"));
					inforRating.setStatus(rec.getInt("status"));
					...
					inforRating.setInformation(information);
